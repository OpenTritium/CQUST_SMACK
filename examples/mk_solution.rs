...
use cqust_smack_server::{Llm, Options, TopicType};
use dashmap::DashMap;
...
async fn main() -> Result<()> {
    ...
    stream
        ...
        .for_each_concurrent(2, |entry| {
            let llm_c = llm.clone();
            let sln_c = solutions.clone();
            async move {
                let (k, v) = entry.expect("failed to unwrapper cache db entry");
                let topic = serde_json::from_slice::<TopicType>(k.as_slice()).expect("failed to deser topic");
                let options = serde_json::from_slice::<Options>(v.as_slice()).expect("failed to deser options");
        ...
        .await;
    let mut f = File::open("./solution_mapping.json")?;
